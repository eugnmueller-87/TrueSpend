...
import {
  POSTGREST_URL, POSTGREST_JWT, N8N_WEBHOOK, N8N_WEBHOOK_BASE,
  pgFetch, pgPost, pgPatch, pgRpc, postIntake, postWebhook, boardAction,
  routeMoneyThroughN8n,
} from './api/client'

// ─── Config + API client ───────────────────────────────────────────────────────
// All browser->backend calls funnel through ./api/client (the Phase 2 chokepoint).
// Config resolution (runtime /config.js → VITE_* → fallback) lives there too.
    ...
    try {
      ...
      if (action === 'approve' || action === 'confirm') {
        ...
        await boardAction({
          ...
          // When routed via n8n, only the ticket id crosses the wire; the
          // board-action workflow re-fetches the ticket fields server-side.
          webhookAction: 'approve',
          webhookBody:   { ticket_id: full.id, actor: boardRow.owner_id || null },
        })
        ...
        await boardAction({
          ...
          webhookAction: 'reject',
          webhookBody:   { ticket_id: id },
        })

      } else if (action === 'ack') {
        // close_ticket RPC: sets status='closed' — SECURITY DEFINER
        await boardAction({
          rpcName: 'close_ticket',
          rpcBody: { p_ticket_id: id },
          webhookAction: 'close',
          webhookBody:   { ticket_id: id },
        })
      }
...
const OrdersBoard = ({ onCountChange, user }) => {
  ...
  const handleMarkDelivered = async (po) => {
    ...
    try {
      await boardAction({
        rpcName: 'confirm_delivery',
        rpcBody: { p_po_id: po.id, p_confirmed_by: user?.email || '' },
        webhookAction: 'confirm_delivery',
        webhookBody:   { po_id: po.id, confirmed_by: user?.email || '' },
      })
      await load()
    } catch (e) {
  ...
  const handleMatchInvoice = async (po) => {
    ...
    try {
      ...
      const result = await boardAction({
        rpcName: 'match_invoice',
        rpcBody: { p_invoice_id: inv.id },
        webhookAction: 'match_invoice',
        webhookBody:   { invoice_id: inv.id, po_id: po.id },
      })
      const matchLabel = result?.match_result || (Array.isArray(result) ? result[0]?.match_result : null) || 'unknown'
  ...
  const handleCreatePayment = async (po) => {
    ...
    try {
      ...
      await boardAction({
        rpcName: 'create_payment_instruction',
        rpcBody: { p_invoice_id: inv.id, p_due_date: dueDate },
        webhookAction: 'create_payment',
        webhookBody:   { invoice_id: inv.id, due_date: dueDate },
      })
...
const BudgetScreen = ({ user }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Budget envelope writes are money-relevant (I-1). When the server-side
      // route is enabled they go through /webhook/board-action (server JWT);
      // otherwise direct via the chokepoint helper (legacy).
      if (routeMoneyThroughN8n) {
        await postWebhook('/board-action', { action: 'budget_upsert', ...payload })
      } else {
        await pgPost('/budget_positions', payload)
      }
  ...
  const handleEditSave = async (row) => {
    try {
      const newBudget = parseFloat(editRow.planned) || 0
      if (routeMoneyThroughN8n) {
        await postWebhook('/board-action', { action: 'budget_upsert', id: row.id, budget: newBudget })
      } else {
        await pgPatch(`/budget_positions?id=eq.${row.id}`, { budget: newBudget })
      }
...
const UsersScreen = () => {
  ...
  const saveUser = async () => {
    ...
    try {
      // User create/role/active are AUTH-sensitive (I-2: this table feeds the
      // app_role claim post-SSO). Route through the server-side workflow when
      // enabled so the browser token cannot mint or elevate a user directly.
      const newUser = {
        name:      form.name.trim(),
      ...
      if (routeMoneyThroughN8n) {
        await postWebhook('/board-action', { action: 'user_create', ...newUser })
      } else {
        await pgPost('/users', newUser)
      }
      setAdding(false)
  ...
  const toggleActive = async (u) => {
    try {
      if (routeMoneyThroughN8n) {
        await postWebhook('/board-action', { action: 'user_set_active', id: u.id, active: !u.active })
      } else {
        await pgPatch(`/users?id=eq.${u.id}`, { active: !u.active })
      }
      load()
    } catch {}
  }
  ...
  const changeRole = async (u, newRole) => {
    ...
    try {
      if (routeMoneyThroughN8n) {
        await postWebhook('/board-action', { action: 'user_set_role', id: u.id, role: newRole })
      } else {
        await pgPatch(`/users?id=eq.${u.id}`, { role: newRole })
      }
      load()
    } catch {}
  }
